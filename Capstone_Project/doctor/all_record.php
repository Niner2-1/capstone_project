...
<html lang = "en">
	...
	<div class = "well">
		...
		<div class = "panel panel-primary">
			<?php
				$q1 = $conn->query("SELECT * FROM `patient_record` WHERE `itr_no` = '$_GET[itr_no]'") or die(mysqli_error());
				$f1 = $q1->fetch_array();
			?>
			<div class = "panel-heading">
			...
			<?php 
				$conn = new mysqli("localhost", "root", "", "capstonedbdraft") or die(mysqli_error());
				$q = $conn->query("SELECT * FROM `patient_record` WHERE `itr_no` = '$_GET[itr_no]' ORDER BY `record_date` DESC") or die(mysqli_error());
				while($f = $q->fetch_array()){
			?>
				<tr>
					<td><?php echo $f['record_date']?></td>
